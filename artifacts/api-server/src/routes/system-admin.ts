...
router.post("/system/restore", requireAdmin, async (req, res): Promise<void> => {
  ...
  // Company mismatch is intentionally not blocked: the restore already scopes
  // all deletes and inserts to the caller's own company_id, so there is no
  // cross-tenant data leak. Admins may restore any backup into their company.
